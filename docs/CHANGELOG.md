...
### Fixed
- **Migrate from deprecated budget_tokens to adaptive thinking** — Sonnet 4.6 deprecates `thinking: {"type": "enabled", "budget_tokens": N}`. Using the deprecated API caused 100+ minute stalls on single engine calls (78 min thinking phase before any text output, then ~1 token/sec). Migrated to GA adaptive thinking: `thinking: {"type": "adaptive"}` with `output_config: {"effort": "medium|high"}`. No beta header needed for thinking (only 1M context still uses beta). Changed in both `engine_runner.py` (MODEL_CONFIGS, `_execute_streaming_call()`) and `planner.py` (plan generation + refinement). Effort levels: "high" for opus-tier deep analysis, "medium" for standard sonnet calls, None for haiku. ([`src/executor/engine_runner.py`](src/executor/engine_runner.py), [`src/orchestrator/planner.py`](src/orchestrator/planner.py))
