...
### Added
- **Conditions of Possibility child views** — Created 4 child view definitions surfacing CoP sub-components as visible lego blocks in the view tree instead of burying them in section_renderers config: `genealogy_cop_enabling_conditions` (card_grid), `genealogy_cop_constraining_conditions` (card_grid), `genealogy_cop_counterfactual` (prose), `genealogy_cop_synthesis` (prose). Each references the `conditions_of_possibility_analyzer` engine with appropriate `result_path`. Follows the same child-view pattern established by Target Work Profile's 4 sub-views. ([`src/views/definitions/genealogy_cop_*.json`](src/views/definitions/))
