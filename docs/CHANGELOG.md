...
- **Pre-split chapter upload** — Users can upload individual chapters alongside the full target work text at analysis start time via the new `target_work_chapters` field on `AnalyzeRequest`. Each chapter is stored as its own document (role="chapter") and keyed as `"chapter:{chapter_id}"` in `document_ids`. Chapter-targeted phase execution (`_run_chapter_targeted_phase()`) now checks for pre-uploaded chapter documents first, falling back to char-offset extraction only when no pre-uploaded chapter exists. The book sampler uses pre-uploaded chapter metadata directly (skipping regex detection) when available. ([`src/orchestrator/pipeline_schemas.py`](src/orchestrator/pipeline_schemas.py), [`src/orchestrator/pipeline.py`](src/orchestrator/pipeline.py), [`src/executor/phase_runner.py`](src/executor/phase_runner.py), [`src/orchestrator/sampler.py`](src/orchestrator/sampler.py))
