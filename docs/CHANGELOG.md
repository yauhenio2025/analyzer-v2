...
- **V2TabContent merges fresh view definitions' renderer_config** — V2TabContent (the V2 presentation rendering path) now fetches fresh view definitions via `useViewDefinitions` and merges missing keys (like `section_renderers`) into the stored PagePresentation's renderer_config. This means view definition updates take effect immediately without requiring re-import. Enhanced sub-renderer styling: MiniCardList cards have indigo left accent border, pill-style subtitle/badge, ProseBlock has purple left border. ([`the-critic/webapp/src/pages/GenealogyPage.tsx`], [`the-critic/webapp/src/components/renderers/SubRenderers.tsx`])
